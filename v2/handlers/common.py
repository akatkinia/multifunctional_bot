...
async def cb_currency_cbrf(callback: types.CallbackQuery, callback_data: dict, state: FSMContext):
    if callback_data['command'] == 'cb_currency_cbrf':
        await ProfileStatesGroup.currency.set()
        await callback.message.edit_text("Напишите мнемокод валюты, курс которой хотите узнать (например <code>USD</code>). Для получения списка мнемокодов, вызовите справку", parse_mode="HTML", reply_markup=currency_ikb())
...
async def cb_mir_currency(callback: types.CallbackQuery, callback_data: dict, state: FSMContext):
    if callback_data['command'] == 'cb_currency_mir':
        await ProfileStatesGroup.currency_mir.set()
        await callback.message.edit_text(f"Выберите способ отображения данных.\nЕсли требуется информация о доступных на сегодняшний день валютах и диапазонах дат, вызовите справку", parse_mode="HTML", reply_markup=mir_currency_ikb())
...
def register_handlers_common(dp: dp):
    ...
    dp.register_callback_query_handler(cb_cat_cancel, cb.filter(command='cancel'), state='*')
